...
This firmware adds an entirely separate event processing layer on top of basic QMK features.
...
My choice for my first keybuard fell on the Keychron K11 Max, since it offered good connectivity, good support (QMK) and came in an Alice layout.
...
*Update:* Eventually, I built my own split wired keyboard with the same user space and bindings.
...
E.g., I previously used `CTRL+E` to switch files, now I use `g fb`.
...
Tapping `REP` repeats the last key. Holding it activates shift.

Double tapping and holding the repeat key switches to the function key layer instead.
I rarely use them, so I don't mind the indirection.

Tapping the `SUF` key enters a ["suffix"](#suffix-mode) mode.

The second alpha layer may alternatively be activated with the right ring finger (hence the keyboard name "magic mirror").
...
I have a "magic" key that performs contextual actions based on what was previously typed.
I have two separate functions for it.
The idea is one acts on only what was typed immediately before:

- Transform "ue", "ae", "oe", "ss" into their german _Umlaute_ "üäöß"
- Expand abbreviations like "eg", "zB", "iferr" :), ...
- Complete words (e.g. imp -> implement) 
...
The suffix mode inserts commen suffixes:
...
Edit: I don't use any of these beside CAPS_WORD, which I use all the time.
For the others, the cognitive "overhead" is too big (at least currently).
